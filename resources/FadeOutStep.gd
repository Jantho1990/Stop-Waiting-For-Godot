...
func next() -> void:
  ...
  for targetNodePath in targetNodes:
    ...
    _slideTween.tween_property(targetNode, "modulate", FADE_OUT_COLOR_END, animation_duriation).from(FADE_OUT_COLOR_BEGIN)
  await _slideTween.finished
#  print('DBG: next finished %s' % [targetNodes[0].modulate])
  _slideTween = null
  step_next_ended.emit()
  print('DBG: fade out done')
...
func previous() -> void:
  ...
  for targetNodePath in targetNodes:
    var targetNode = slide.get_node(targetNodePath)
    _slideTween.tween_property(targetNode, "modulate", FADE_OUT_COLOR_BEGIN, animation_duriation).from(FADE_OUT_COLOR_END)
  await _slideTween.finished
  _slideTween = null
